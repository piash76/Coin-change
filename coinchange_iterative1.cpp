...
//from a loser
//ll a[maxn];
///number of ways to make a value by taking coins (taking each coin once ) ///subset sum ///retrun yes or no
ll coins[5000];
bool dp[5000][5000];
...
ll coin_change(ll value,ll coin_no)
{
    ...
    //cout<<coin_no<<endl;
	for(ll i=0;i<=coin_no;i++) dp[i][0]=1;
    ...
    {
        for(ll j=0;j<=value;j++) cout<<dp[i][j]<<" ";
        cout<<endl;
    }
	...
	/*if(dp[coin_no][value]){
            cout<<"YES"<<endl;
            ll i=coin_no,j=value;
            while(i && j){
                if(dp[i-1][j]){
                    i--;
                    continue;
                }
                if(i!=coin_no && j!=value) cout<<" ";
                cout<<coins[i];
                j-=coins[i],i--;
            }cout<<endl;
	}
...
int main()
{
   ...
   cin>>t;
    while(t--)
    {
        ...
        ll coin_no;
        cin>>coin_no;
        for(i=1;i<=coin_no;i++) cin>>coins[i];   ///indexing from 1
